...
func InitCache(name string, lc logger.LoggingClient, vdc coredata.ValueDescriptorClient, dc metadata.DeviceClient, pwc metadata.ProvisionWatcherClient) {
	initOnce.Do(func() {
		...
		ds, err := dc.DevicesForServiceByName(ctx, name)
		if err != nil {
		...
		pws, err := pwc.ProvisionWatchersForServiceByName(ctx, name)
		if err != nil {
