...
func (c *RestController) discoveryFunc(w http.ResponseWriter, req *http.Request, dic *di.Container) {
	...
	handler.DiscoveryHandler(w, discovery, lc)
}
...
func (c *RestController) transformFunc(w http.ResponseWriter, req *http.Request, dic *di.Container) {
	...
	vars := mux.Vars(req)
	lc := bootstrapContainer.LoggingClientFrom(dic.Get)
	_, appErr := handler.TransformHandler(vars, lc)
	if appErr != nil {
...
func (c *RestController) commandFunc(w http.ResponseWriter, req *http.Request, dic *di.Container) {
	...
	if appErr != nil {
		...
		eventClient := container.CoredataEventClientFrom(dic.Get)
		...
		// push to Core Data
		go common.SendEvent(event, c.LoggingClient, eventClient)
	}
}
		...
		for _, event := range events {
			if event != nil {
				go common.SendEvent(
					event,
					c.LoggingClient,
					container.CoredataEventClientFrom(dic.Get))
			}
		}
		w.Header().Set(clients.ContentType, clients.ContentTypeJSON)
...
func (c *RestController) metricsFunc(w http.ResponseWriter, _ *http.Request, dic *di.Container) {
