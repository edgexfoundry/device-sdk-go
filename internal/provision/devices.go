...
func LoadDevices(deviceList []common.DeviceConfig, dic *di.Container) error {
	...
	for _, d := range deviceList {
		if _, ok := cache.Devices().ForName(d.Name); ok {
			...
			if err != nil {
				lc.Error(fmt.Sprintf("creating Device from config failed: %s", d.Name))
				return err
			}
		}
	}
...
func createDevice(dc common.DeviceConfig, lc logger.LoggingClient, ds contract.DeviceService, mdc metadata.DeviceClient) error {
	...
	if err != nil {
		lc.Error(fmt.Sprintf("Add Device failed %s, error: %v", device.Name, err))
		return err
	}
	if err = common.VerifyIdFormat(id, "Device", lc); err != nil {
		return err
	}
	device.Id = id
