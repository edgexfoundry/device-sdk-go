...
func (s *DeviceServiceSDK) AddDeviceProfile(profile contract.DeviceProfile) (id string, err error) {
	...
	if err = common.VerifyIdFormat(id, "Device Profile", s.LoggingClient); err != nil {
		return "", err
	}
	profile.Id = id
	_ = cache.Profiles().Add(profile)

	provision.CreateDescriptorsFromProfile(&profile, s.LoggingClient, s.edgexClients.GeneralClient, s.edgexClients.ValueDescriptorClient)
